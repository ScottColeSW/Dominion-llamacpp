# Dominion — Engine Design Notes & History

> **A note on paths in this document.** This file was originally
> `prototype/README.md`, written against that layout (`engine/`,
> `server.py`, `web/`, run via `python3 server.py`). The port to a real
> Python package moved everything under `src/dominion/` (see the root
> [`README.md`](../README.md) for the current layout, how to run either
> the Ollama or llama.cpp backend, and the actual install/run commands).
> Below this note, file paths and run commands are left exactly as
> originally written — they're a historical record of engine design
> decisions and rationale (module names map predictably: `engine/x.py` is
> now `src/dominion/engine/x.py`, `engine/agents.py` is now
> `src/dominion/agents/scripted_agent.py`, `engine/ollama_agent.py` is now
> `src/dominion/agents/llm_agent.py`) — not an operational guide. Rewriting
> every path reference below for the sake of it would cost the history
> without adding anything the root README doesn't already say better.
...
## Run it (original instructions -- see root README.md for the current ones)
...
```
pytest tests/test_board_geometry.py -v   # was: python -m unittest engine.test_board_geometry -v
```
